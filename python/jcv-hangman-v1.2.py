...
# - Replaced location_test() with
#    find_location() and add_letter()
# - Updated words list
##################
#2016/12/13
# V1.2
# - Added animations for winnning and losing
...
#Import msvcrt for user input polling
# during winner animations
...
#Statement to select random word
# from the words list
word = random.choice(words)

#List to hold all letters that
# have been entered to guess
bucket = []

#List to hold guessed letters that
# are found in mystery word
build_word = []

#List to hold guessed letters that
# are not found in mystery word
wrong_count = []

#Variables to hold cumulative score
# (games won and lost)
games_won = 0
games_lost = 0
...
#Function that acts as primary starting/return point
# for other functions
def user_input():
	#Display the cumulative score
	score()
	#Display the current progressive hangman
	# "image" based on wrong guesses
	hangman(len(wrong_count))
	print(" ")
	#Display guessed/total letters
	letters()
	#Using "for x in range()" so it's faster
	# to adjust margins
	for x in range(2):
		print(" ")
	#Prompt for collecting a letter from the user
	letter = input("  Please enter a letter:")
	#Pass the user-specified letter to the
	# good_letter function
	good_letter(letter)

#Function that checks the user-specified letter
# for a few things
def good_letter(a):
	...
	if a in bucket:
		...
		user_input()
	#Check if a single -letter- has been entered
...
def letter_test(b):
	#If it is in the word, pass it to the
	# location_test function
	if b in word:
		find_location(word, b)
	#If it is not in the word, pass it to the
	# wrong_letter function
	else:
		wrong_letter(b)

#Function that receives mystery word and user letter from
# letter_test function, checks to find all locations of letter
# in word, and writes location index to found list and passes
# to add_letter function
def find_location(word, letter):
	#List to hold the index (position) for 
	# all instances of the letter in the word
	location = []
	#Assign the index to -1 so it will become 0
	# when evaluated in the try clause
	#When set at 0, the index() function will miss the
	# letter if it is the first character in the word
	last_index = -1
	while True:
		try:
			#string.index(letter_to_find,
			# place_to_start_looking)
			#Increment the value of last_index
			# (from -1 to 0 on first pass) and then
			# assign last_index to to the index
			# (position) of the letter
			last_index = word.index(letter, last_index+1)
		except ValueError:
			#If the letter is not found in the word,
			# it will return ValueError and this loop
			# should end without writing anything to
			# the location list
			#Note that letter_test should preclude this
			break
		else:
			#Append the index (position) of the letter
			# to the location list on each pass
			#If the letter occurs multiple times, this 
			# list will have multiple indexes listed
			location.append(last_index)
	#Pass the user's letter and the location list
	# to the add_letter function
	add_letter(letter, location)

#Receives letter and location list from find_location
# function and adds letter to the corresponding location(s)
# in the build_word list
# Finishes by calling the word_test function with the
# mystery word
...
#Function to compare the current build_word list
# against the mystery word
def word_test(word):
	#Use list() to turn the mystery word into a list
	# and if it matches the build_word list, increase
	# the games_won score by 1 and initiate the winner
	# function with an animation count of 1
	if build_word == list(word):
		global games_won
		games_won += 1
		winner(1)
	#If they don't match, return to user_input()
	# for another letter
	else:
		user_input()
...
def wrong_letter(d):
	...
	# so increase the games_lost score by 1 and initiate
...
#Function to capture key presses for game_over function
def readch():
...
#Check to see if user has pressed a key to start a new game or
# exit the game between each animation frame
...
def game_over(ani_count):
	...
	if msvcrt.kbhit():
		key = ord(readch())
		#If the user presses any key except Esc,
		# select a new word, clear all lists, 
		...
		#If the user presses the Esc key, exit the game
			...
			os.system("cls")
			print("Exiting game...")
	#If the user didn't press a key and the animation count
	# is less than 5, run the winner() animation sequence
	elif ani_count < 5:
		winner(ani_count)
	#If the user didn't press a key and the animation count
	# is 5 or higher, run the loser() animation sequence
	else:
		loser(ani_count)

#Function to print repetitive congratulations text
# in winner() animations
def congratulations():
	for x in range(2):
		print(" ")
...
#Function that is called from the word_test function when
# the build_word list and the mystery word are matching
...
#Function to print repetitive game over text
# in loser() animations
def sorry():
	print(" ")
